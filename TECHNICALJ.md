...
検索ボックスは shadow DOM の奥にあり、しかも `/` を押すまで `input` が**生成されない**。
初期実装は「`document` を再帰走査して最初の `input/textarea` を掴む」方式だったが、
Earth は初回検索後に**不可視の input を DOM に残す**ため、2 回目以降は不可視要素へ
打鍵してしまい「前回の検索語のまま Enter だけ飛んでカメラが動かない」状態になった。
現在の `fly_to()` は投げっぱなしをやめ、各段階を検証する:

1. **地球ビューに居ることを確認**（`ensure_globe()`）。Earth はプロモ表示などの
   巡り合わせで「地図プロジェクト」管理画面に遷移することがある。この画面にも
   検索ボックスがあるため、気づかず検索すると入力だけ通ってカメラが動かない。
   URL からカメラ状態が読めなければ初期位置へ再読込して復帰する。
2. Escape で残パネルを閉じ `/` で検索を開き、**その直後の activeElement**
   （shadow root を辿る）だけを入力対象にする。消去は実キーイベント
   （Ctrl+A, Delete）で行う — `.value=''` は Earth 内部の検索状態に旧語が残り
   追記されてしまう。
3. 打鍵後に**値を読み戻して確認**。文字が欠落していたら（オートコンプリート
   再描画と競合しうる）`keyboard.insert_text()` で再試行する。
4. Enter 後に**着地を検証**。カメラ距離が 1,000km を下回り、かつ**飛行前から
   実際に変化した**こと（URL に前回座標が残るだけの誤検知を防ぐ）。
5. 全試行が失敗したら `earth.google.com/web/search/<地名>` へ `page.goto` する
   テレポートにフォールバック（アニメーションは失うが展示が無言で止まらない）。

到着後は `dismiss()`（Escape）で情報パネルを閉じる。

起動時も同様に「仮定せず検証」する。従来の WASM ブート待ちは固定 13 秒で、
`start_all.sh` のコールドスタートでは llama の 22GB ロードが CPU を食って
13 秒で Earth が起動しきらず、以後のコマンドが半起動のアプリに落ちて全滅した。
`goto_home()` は「URL のカメラが初期位置を示し、かつ Earth が初期化完了時に
自分で付ける `/data=` サフィックスが現れる」まで待ち、駄目なら最大 3 回リロードする。
...
> （`server.py` の `SYSTEM_PROMPT` = コテコ／一人称コテコ／元気のいい女の子口調）を**上書き**する。
...
- **PyTorch (ROCm) は gfx1151 専用ホイールが必須**。`repo.amd.com/rocm/whl/gfx1151/` の
  ...
  **全 GPU 操作が落ちる**。torch 同梱の `rocm-sdk-libraries-gfx1151`（7.12）は、
  システム SDK を ROCm 10 に更新しても維持する。CTranslate2 は torch がロードした
  ランタイムを共有し得るため、システム SDK を使うと決めつけず、
  `ttllm/verify_rocm.py` で実際のライブラリのロード先を確認する。
...
- **three-vrm は venv python で起動**する。Ubuntu 26.04 の system python（3.14）には
  `aiohttp` が無い。`start_all.sh` は `TTLLM_VENV`（既定: `ttllm/.venv`）→
  `WHISPERX_VENV`（既定: `~/whisperx/whisperX-rocm/.venv`）→
...
- [x] **演出仕上げ**: 字幕の 3 行制限＋音声同期スクロール / `/clear` による字幕一括クリア /
  ツアー無限ループ（`loop`）と起動・停止スクリプト / `world.json` の口調をコテコ・元気のいい女の子口調に統一
